...
def init_db() -> None:
    ...
    db.execute(
        "INSERT OR IGNORE INTO users (username, password, role, is_active, expires_at) VALUES ('admin','admin123','ADMIN',1,NULL)"
    )
    db.execute("UPDATE users SET role = 'ADMIN', is_active = 1, expires_at = NULL WHERE username = 'admin'")
    db.commit()
    db.close()
